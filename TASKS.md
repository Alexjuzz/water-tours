...
Ветка: `main`. Backend: Spring Boot 3.5.6, Java 21, PostgreSQL, Redis, Maven (`mvnw`).

## Ревизия от 2026-09-06

Проверил построчно, что из старого чеклиста реально реализовано в коде (не по названию коммита, а по факту —
читал каждый файл). Итог: **пункты 1–5, 7–10, 17 сделаны правильно и по существу** — это не косметика, реальная
рабочая логика (проверка статуса платежа через API ЮKassa вместо доверия телу вебхука, экранирование XSS,
access-token на заказ, идемпотентность и т.д.). `./mvnw -q -o compile` проходит зелёным.

Но нашёл и сразу исправил 2 реальных бага, которые не были заметны на первый взгляд:
- **№11 фактически не работал**: метод `@ExceptionHandler(StillProcessingException.class)` был вставлен внутрь
  самого класса `StillProcessingException`, а не в `@RestControllerAdvice GlobalExceptionHandler` — Spring
  такой метод никогда не вызывает (`@ExceptionHandler` работает только в контроллере/`@ControllerAdvice`).
  Перенёс в `GlobalExceptionHandler`, теперь работает.
- **Опечатка в `application.yml`**: `management.health.redis.enabled: true1` (лишняя `1`) — некорректное
  значение для `Boolean`-свойства, могло падать при старте приложения на биндинге конфигурации. Исправлено на `true`.
- Пункт **№19** («убрать посторонний zip») был явно указан в чеклисте, но проигнорирован — файл всё ещё лежал
  в исходниках. Удалил.

Вывод: критичная работа (безопасность, оплата) сделана добросовестно, но результат **не был перепроверен перед
сдачей** — остались нерабочий кусок кода (скопирован не в тот класс) и опечатка в конфиге, плюс проигнорирован
один явно указанный пункт уборки. Итог по чеклисту ниже актуализирован под реальное состояние кода.
...
- [x] 1. Проверка подлинности webhook оплаты — проверено, статус берётся через `GET /v3/payments/{id}`, не из тела запроса
- [x] 2. Реальная интеграция создания платежа с ЮKassa — проверено, `paymentUrl` реальный, `providerPaymentId` сохраняется
- [x] 3. Экранирование HTML на странице проверки билета (XSS) — проверено, `HtmlUtils.htmlEscape` + валидация `code`
- [x] 4. Включить CSRF для сессионных (staff) эндпоинтов — проверено, публичные API исключены точечно
- [x] 5. Не отдавать текст исключения клиенту на 500-ошибках — проверено, фиксированная фраза + полный стектрейс в лог
- [x] 6. Активировать Redis-профиль идемпотентности — `SPRING_PROFILES_ACTIVE:redis` по умолчанию
- [x] 7. Сохранять `idempotencyCode` заказа — проверено, есть unique-констрейнт на колонке
- [x] 8. Запрет заказа с нулевым количеством билетов — проверено, бросает `IllegalArgumentException`
- [x] 9. Токен доступа к заказу для публичных эндпоинтов — проверено, `accessToken` + `checkAccess`
- [x] 10. 400 вместо 500 на некорректный JSON/enum в теле запроса — проверено
- [x] 11. Обработка `StillProcessingException` — **был сломан (хендлер не в том классе), исправлено 2026-09-06**
- [x] 12. Секреты и учётные данные персонала — вынесены в `application.yml`/env, литералов в коде не осталось
- [ ] 13. Rate limiting на создание заказа/редим/логин — не начато
- [~] 14. Логирование бизнес-событий + Actuator — Actuator подключен и настроен; логирование есть только в `PaymentService`, в `OrderService`/`TicketService` логгеров нет
- [x] 15. Health-check через Actuator (проверка БД/Redis) — `management.health.db/redis.enabled: true` (опечатка `true1` исправлена 2026-09-06)
- [ ] 16. Базовый набор unit/integration тестов — не начато, есть только пустой сгенерированный `WaterToursApplicationTests`
- [x] 17. Job истечения заказов (`EXPIRED`) — проверено, `OrderExpirationJob` реализован корректно
- [ ] 18. Flyway-миграции + прод-профиль + CI — не начато
- [x] 19. Удалить посторонний zip-архив — было проигнорировано, удалено 2026-09-06
- [ ] 20. Удалить/доработать `TicketInit` — не начато, файл всё ещё на месте
- [ ] 21. Цены/скидки — в конфигурацию — не начато, `TicketProperties` всё ещё статические поля
- [ ] 22. Удалить мёртвый закомментированный код — частично: убрана закомментированная зависимость `spring-boot-docker-compose`; `spring-security-test` в `pom.xml` всё ещё закомментирована
